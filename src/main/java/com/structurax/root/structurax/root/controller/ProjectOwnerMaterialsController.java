...
public class ProjectOwnerMaterialsController {
    ...
    public ResponseEntity<Map<String, Object>> uploadPaymentReceipt(
        ...
        Map<String, Object> response = new HashMap<>();
        try {
            logger.info("Uploading payment receipt for project_id={}, installment_id={}", projectId, installmentId);
            ...
            Files.write(filePath, receiptFile.getBytes());
            ...
            receiptDTO.setReceiptFilePath(filePath.toString());
            receiptDTO.setStatus("Pending");

            PaymentReceiptDTO uploaded = materialsService.uploadPaymentReceipt(receiptDTO);
            response.put("success", true);
            response.put("receipt", uploaded);
